...
- A single file directly under `homeassistant/components/`, such as `automation.py`, is a compat module: it answers what integrations ask a removed layer and is not an integration. List it under `compat_modules` in the closure config, and keep it an answer, never a copy of the product. A manifest must never name it in `dependencies` or `after_dependencies`: setup of the naming integration fails on it, and the closure gate reports it (ADR 0020).
