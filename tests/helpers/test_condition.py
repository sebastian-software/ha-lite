...
async def test_numeric_state_raises(hass: HomeAssistant) -> None:
    ...
    hass.states.async_set("number.missing", "number")
    with pytest.raises(
        ConditionError,
        match="'below'.*number.missing.*cannot be processed as a number",
    ):
        test.async_check()
    ...
    with pytest.raises(
        ConditionError,
        match="'above'.*number.missing.*cannot be processed as a number",
    ):
        test.async_check()
...
async def test_state_condition_attr_duration_initial_state(
    ...
    """Test attribute-based condition initialization.

    The condition anchors `for:` durations to the current state's
    last_updated. This is conservative: when the main state changes but the
    tracked attribute stays the same, last_updated is bumped and the effective
    duration resets (see the `state_change_bumps_last_updated_not_met` case).
    """
    for step in steps:
