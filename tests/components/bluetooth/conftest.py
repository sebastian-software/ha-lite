...
@pytest.fixture
def integration_matchers() -> list[dict[str, str]]:
    """Return the integration matchers to set up bluetooth with."""
    return []


@pytest.fixture
def mock_integration_matchers(
    integration_matchers: list[dict[str, str]],
) -> Generator[None]:
    """Set up bluetooth with the test's integration matchers.

    Request it before enable_bluetooth, which is where bluetooth is set up.
    """
    with patch(
        "homeassistant.components.bluetooth.async_get_bluetooth",
        return_value=integration_matchers,
    ):
        yield
