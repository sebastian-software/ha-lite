...
async def test_frontend_has_no_dedicated_startup_stage(hass: HomeAssistant) -> None:
    """Test frontend is not prioritized ahead of stage 0."""
    order = []
    ...
    mock_integration(
        ...
        MockModule(
            domain="zeroconf",
            async_setup=gen_domain_setup("zeroconf"),
        ),
    )
    ...
    assert "normal_integration" in hass.config.components
    assert "zeroconf" in hass.config.components
    assert "http" in hass.config.components

    # Frontend is handled with normal stage-2 integrations and must no longer
    # be prioritized ahead of stage 0.
    assert order.index("zeroconf") < order.index("frontend")
