...
Recovery mode must be redefined as an operational/API recovery path. **Still open (#30).** `DEFAULT_INTEGRATIONS_RECOVERY_MODE` in `homeassistant/bootstrap.py` is still Home Assistant's recovery mode with the product entries removed, not a headless recovery path of its own.
